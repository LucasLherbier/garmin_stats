...
def get_recent_activities_query(sport_type, timerange):
    time_filters = {
        '8_weeks': {
            'start': 'DATE_TRUNC(DATE_SUB(CURRENT_DATE(), INTERVAL 84 DAY), WEEK(MONDAY))',
            'end': 'DATE_TRUNC(CURRENT_DATE(), WEEK(MONDAY))'
        },
        '6_months': {
            'start': 'DATE_TRUNC(DATE_SUB(CURRENT_DATE(), INTERVAL 6 MONTH), WEEK(MONDAY))',
            'end': 'DATE_TRUNC(CURRENT_DATE(), WEEK(MONDAY))'
        },
        'ytd': {
        ...
        'all': {
            'start': f'(SELECT DATE_TRUNC(MIN(DATE(Week)), WEEK(MONDAY)) FROM {ACTIVITIES} WHERE activityTypeGrouped = "running")',
            'end': f'(SELECT DATE_TRUNC(MAX(DATE(Week)), WEEK(MONDAY)) FROM {ACTIVITIES} WHERE activityTypeGrouped = "running")'
        }
    }
...
def get_weekly_sport_query(sport_type, timerange):
    time_filters = {
        '8_weeks': {
            'start': 'DATE_SUB(CURRENT_DATE(), INTERVAL 84 DAY)',
            'end': 'CURRENT_DATE()'
        },
        '6_months': {
            'start': 'DATE_SUB(CURRENT_DATE(), INTERVAL 6 MONTH)',
            'end': 'CURRENT_DATE()'
        },
        'ytd': {
        ...
        'all': {
            'start': f'(SELECT MIN(DATE(Week)) FROM {ACTIVITIES})',
            'end': f'(SELECT MAX(DATE(Week)) FROM {ACTIVITIES})'
        }
    }
    start_date = time_filters[timerange]['start']
    end_date = time_filters[timerange]['end']

    date_series_sql = f"UNNEST(GENERATE_DATE_ARRAY(DATE_TRUNC({start_date}, WEEK(MONDAY)), DATE_TRUNC({end_date}, WEEK(MONDAY)), INTERVAL 7 DAY))"

    if sport_type == "duration":
        return f"""
            WITH date_series AS (SELECT Week FROM {date_series_sql} AS Week)
            SELECT
                ds.Week,
    ...
    return f"""
        WITH week_series AS (
            ...
            FROM UNNEST(GENERATE_DATE_ARRAY(
                DATE_TRUNC(DATE(EXTRACT(YEAR FROM CURRENT_DATE()), 1, 1), WEEK(MONDAY)),
        ...
        activity_weeks AS (
            SELECT
                DATE_TRUNC(DATE(startTimeLocal), WEEK(MONDAY)) AS week,
    ...
    return f"""
        ...
        full_weeks AS (
            SELECT
                ...
                COALESCE(aw.distance, 0) AS distance,
                COALESCE(aw.calories, 0) AS calories,
                COALESCE(aw.totalNumberOfStrokes, 0) AS totalNumberOfStrokes,
                COALESCE(aw.total_hr_duration, 0) AS total_hr_duration,
                RANK() OVER (ORDER BY ws.week DESC) AS rank_week
            FROM week_series ws
            LEFT JOIN activity_weeks aw ON ws.week = aw.week
        ...
        SELECT 'last_1' AS name, SUM(duration) AS duration_total, SUM(duration)/NULLIF((SELECT c1 FROM counts), 0) AS duration_avg, SUM(nb_trainings) AS nb_trainings, SUM(distance) AS distance_total, SUM(distance)/NULLIF((SELECT c1 FROM counts), 0) AS distance_avg, SUM(calories) AS calories, SUM(totalNumberOfStrokes) AS totalNumberOfStrokes, CAST(SUM(total_hr_duration) / NULLIF(SUM(duration),0) AS INT64) AS averageHR FROM full_weeks WHERE rank_week <= 1
        UNION ALL
        SELECT 'last_4' AS name, SUM(duration) AS duration_total, SUM(duration)/NULLIF((SELECT c4 FROM counts), 0) AS duration_avg, SUM(nb_trainings) AS nb_trainings, SUM(distance) AS distance_total, SUM(distance)/NULLIF((SELECT c4 FROM counts), 0) AS distance_avg, SUM(calories) AS calories, SUM(totalNumberOfStrokes) AS totalNumberOfStrokes, CAST(SUM(total_hr_duration) / NULLIF(SUM(duration),0) AS INT64) AS averageHR FROM full_weeks WHERE rank_week <= 4
        UNION ALL
        SELECT 'last_12' AS name, SUM(duration) AS duration_total, SUM(duration)/NULLIF((SELECT c12 FROM counts), 0) AS duration_avg, SUM(nb_trainings) AS nb_trainings, SUM(distance) AS distance_total, SUM(distance)/NULLIF((SELECT c12 FROM counts), 0) AS distance_avg, SUM(calories) AS calories, SUM(totalNumberOfStrokes) AS totalNumberOfStrokes, CAST(SUM(total_hr_duration) / NULLIF(SUM(duration),0) AS INT64) AS averageHR FROM full_weeks WHERE rank_week <= 12
        UNION ALL
        SELECT 'last_18' AS name, SUM(duration) AS duration_total, SUM(duration)/NULLIF((SELECT c18 FROM counts), 0) AS duration_avg, SUM(nb_trainings) AS nb_trainings, SUM(distance) AS distance_total, SUM(distance)/NULLIF((SELECT c18 FROM counts), 0) AS distance_avg, SUM(calories) AS calories, SUM(totalNumberOfStrokes) AS totalNumberOfStrokes, CAST(SUM(total_hr_duration) / NULLIF(SUM(duration),0) AS INT64) AS averageHR FROM full_weeks WHERE rank_week <= 18
        UNION ALL
        SELECT 'last_all' AS name, SUM(duration) AS duration_total, SUM(duration)/NULLIF((SELECT call FROM counts), 0) AS duration_avg, SUM(nb_trainings) AS nb_trainings, SUM(distance) AS distance_total, SUM(distance)/NULLIF((SELECT call FROM counts), 0) AS distance_avg, SUM(calories) AS calories, SUM(totalNumberOfStrokes) AS totalNumberOfStrokes, CAST(SUM(total_hr_duration) / NULLIF(SUM(duration),0) AS INT64) AS averageHR FROM full_weeks;
    """
...
def get_race_metrics_query(start_date, end_date):
    return f"""
        ...
        monthly_stats AS (
            SELECT FORMAT_DATE('%Y-%m', DATE(startTimeLocal)) AS month, SUM(CASE WHEN activityTypeGrouped = 'swimming' THEN distance ELSE 0 END) AS month_swim_distance, SUM(CASE WHEN activityTypeGrouped = 'cycling' THEN distance ELSE 0 END) AS month_bike_distance, SUM(CASE WHEN activityTypeGrouped = 'running' THEN distance ELSE 0 END) AS month_run_distance FROM race_activities GROUP BY month
        ),
        last_8_weeks AS (
